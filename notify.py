...
for host in hosts:
    ...
    isDown = db.conn.execute(
        'SELECT CASE WHEN count(*) > 0 and count(*) = count(responseTime) THEN FALSE ELSE TRUE END AS isDown FROM (SELECT * FROM history WHERE endpoint=? ORDER BY startedOn DESC LIMIT ?)',
